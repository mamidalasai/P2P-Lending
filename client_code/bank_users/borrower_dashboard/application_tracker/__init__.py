...
class application_tracker(application_trackerTemplate):
  def __init__(self, **properties):
    ...
    self.user_id=1000
    rejected_loans = app_tables.loan_details.search(status='rejected', user_id=self.user_id)
    open_form('bank_users.borrower_dashboard.application_tracker.borrower_rejected')
